...
# ╔═╡ 8f671b34-1264-11ec-353e-99ffc7a43db8
begin
	using CSV
	using Optim
	# using Query
...
# ╔═╡ ef979bc9-e573-47b1-939d-e2d2ef132d13
md"""## mechanism transition example."""

# ╔═╡ e31b3b88-abe7-4567-aa53-5103ae1b7f25
begin
	s0 = 30 # asymptotic strength
	k = 250 # inverse sqrt scaling constant
	a = 0.01 # relative grain boundary thickness term
	γ = -0.25 # "interface energy" term
	noise_scale = 70

	N = 50 # synthetic data sample size

	# strengthening model: inverse sqrt scaling
	hallpetch(x) = k / sqrt(x)

	# softening model: 1/x scaling
	inv_hallpetch(x) = γ / (2a * x)

	# plotting grid
	d = 10 .^ collect(-2.6:0.01:2.8)

	# log-uniform random sample of grain size values
	# set the mean to 1e-2.5, variance in log-space to 4.5
	x = 10 .^ (rand(Float64, N) .* 4.5 .- 2.5)

	# simulate a dataset with both strengthening and softening behavior
	ysim = @. s0  + hallpetch(x) + inv_hallpetch(x)
	noise = sqrt.(ysim) .+ noise_scale .* randn(Float64, (length(x),))
	y = ysim .+ noise

	# simulated data
	scatter(x, y, xaxis=:log, label=nothing)

	# ground truth model
	plot!(d, max.(s0 .+ hallpetch.(d) .+ inv_hallpetch.(d), 10), label=L"\sigma")

	# strengthening model
	plot!(d, s0 .+ hallpetch.(d), label=L"1/\sqrt{d}")

	# shift and scaled softening model.
	plot!(d, 1600 .+ 0.3 .* inv_hallpetch.(d), label=L"1/d")
	xlabel!(L"\textrm{grain size } \; (\mu m)")
	ylabel!(L"\sigma \;(MPa)")
	ylims!(0, 2000)
end

# ╔═╡ 2e48432e-a9c0-4780-babb-360c73679898
begin

	function build_model(θ::NamedTuple)
	    return @gppp let
			f1 = θ.mean + θ.s * stretch(GP(SEKernel()), 1 / θ.l)
	    end
	end
	
	θ = (     
	    l = positive(1.0),   # RBF length scale
	    s = positive(100.0), # large variance.
	    s_noise = positive(500.0), # Observation noise variance
		mean = positive(1.0), # constant mean
    )
...
# ╔═╡ 85e21385-d56b-45a8-bdaa-a4e299e1b321
""" negative log marginal likelihood -- objective for GP hyperparameter optimization """
function nlml(θ::NamedTuple)
    f = build_model(θ)
    return -logpdf(f(GPPPInput(:f1, log10.(x)), θ.s_noise + 1e-6), y)
end
...
begin
	...
	f_post_opt = posterior(f_opt(GPPPInput(:f1, log10.(x)), θ_opt.s_noise), y);
end
...
let
	...
	plot!(d, s0 .+ hallpetch.(d), label=L"1/\sqrt{d}", linewidth=2)
	plot!(d, 1600 .+ 0.3 .* inv_hallpetch.(d), label=L"1/d", linewidth=2)
...
git-tree-sha1 = "7c0d3573e745311928819f0fa6a7830799c40892"
...
git-tree-sha1 = "a00a692cd39a8b5f7e780f0327a992c4aa177c67"
...
git-tree-sha1 = "b454231b4559c118fe3522734c71555592445843"
...
[[Stheno]]
deps = ["AbstractGPs", "BlockArrays", "ChainRulesCore", "FillArrays", "KernelFunctions", "LinearAlgebra", "MacroTools", "Random", "Reexport", "Statistics", "Zygote", "ZygoteRules"]
git-tree-sha1 = "9ae442344559f72e352097111650e17a8fdc1e5b"
